package com.vehiclemanagement.db;
...
public class ConnectionFactory {
	...
	public static Connection getConnection() {
		...
		try {
			...
			// throw new
			// MetaHRMSystemException("Could not load driver class, please add mysql jdbc driver to classpath.",
			// e);
		} catch (SQLException e) {
			System.out.println("Coult not create connection with database, ["
					+ e.getMessage() + "]");
			// throw new
			// MetaHRMSystemException("Coult not create connection with database, ["
			// + e.getMessage() + "]", e);
		}
		return connection;
	}
