...
public class Graph {
	 ...
	 * 
	 * @param node
	 * @return 1 if the node is added successfully and -1 if the node is not
	 *         added successfully
	 */
	public int addNode(Node node) {
		try {
			...
			while (iteratorNodeSet.hasNext()) {
				if (iteratorNodeSet.next().getEmailId()
						.equals(entity.getEmailId()))// Email id is unique key
				{
					return -1;
				}
			}
			...
			while (iteratorNodeSet.hasNext()) {
				...
				if (entity.getEmailId().equals(entityToRemove.getEmailId())) {
					...
					if (entityToRemove instanceof Person)
						...
						SocialNetwork.organisationSet
								.remove((Organisation) entityToRemove);
				}
			}
	 ...
	 * @param node1
	 * @param node2
	 * @return 1 if edge is added successfully or return -1 if the edge cannot
	 *         be added (whether the node is not available in the network or
	 *         edge already exists between them)
	 */
	public int addEdge(String name1, String name2) {
		try {
			Entity entityForFristPerson = null;
			Entity entityForSecondPerson = null;
			...
			while (iteratorNodeSet.hasNext()) {
				...
				if (entityTemp.getName().equals(name1)) {
					entityForFristPerson = (Entity) entityTemp;
					firstPersonNameExist = true;
				}
				if (entityTemp.getName().equals(name2)) {
					entityForSecondPerson = (Entity) entityTemp;
					secondPersonNameExist = true;
				}
			}
			if (firstPersonNameExist && secondPersonNameExist) {
				Iterator<Entity> itr = entityForFristPerson.setFriends
						.iterator();
				while (itr.hasNext()) {
					if (itr.next().getEmailId()
							.equals(entityForSecondPerson.getEmailId())) {
						checkForFriend = true;
					}
				}
				if (checkForFriend == true)
					...
					entityForFristPerson.setFriends.add(entityForSecondPerson);
					entityForSecondPerson.setFriends.add(entityForFristPerson);
				}
			} else {
				return -1;
			}
	...
	public void removeEdge(String name1, String name2) {
		try {
			...
			while (iteratorFriends.hasNext()) {
				if (iteratorFriends.next().getEmailId()
						.equals(entity2.getEmailId())) {
	...
	/**
	 * Method to remove an entity from friend list of all other entities if the
	 * entity is removed from the network
	 * 
	 * @param entity
		...
		try {
			...
			while (iteratorNodeSet.hasNext()) {
				...
				while (itrFriends.hasNext()) {
					...
					if (itrFriends.next().getEmailId()
							.equals(entity.getEmailId())) {
