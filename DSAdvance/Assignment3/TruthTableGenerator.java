...
public class TruthTableGenerator {
	...
	private String postfixConvertor(String input) {
		...
		for (int i = 0; i < inputLength; i++) {
			...
			/*
			 * if the character is ')' then pop elements from stack and add them
			 * to postfix String until '(' is encountered and then also pop the
			 * top element i.e.'('
			 */
			else if (temp == ')') {
				if (countBraces == 0) {
			...
			/* if the character is an operator */
				...
				if (stackObj.size() == 0) {
					...
					/*
					 * is current character is '^', then this code will be
					 * executed
					 */
					if (input.charAt(i) == '~') {
					...
					/*
					 * if current character is '*' or '/', then this code will
					 * be executed
					 */
					else if (input.charAt(i) == '&') {
					...
					/*
					 * adding operator to top if the operator has higher
		...
		/*
		 * After complete traversal of input expression, all the elements of
		 * stack are added to postfix string
		 */
